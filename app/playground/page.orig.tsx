...
type PlaygroundSection = 'overview' | 'interactive'
...
type ApiEndpoint = {
  ...
  title: string
  description: string
  category: 'Payments' | 'Refunds' | 'E-NACH'
}
...
type QuickAction = {
  ...
  description: string
  action: string
  category: 'Payment Gateway' | 'Webhooks' | 'E-NACH'
}
...
const apiEndpoints: ApiEndpoint[] = [
  ...
  { method: 'POST', endpoint: '/api/v1/enach/mandates/create', title: 'Create E-NACH Mandate', description: 'Set up recurring payment mandate', category: 'E-NACH' },
  { method: 'GET', endpoint: '/api/v1/enach/mandates/{mandateId}', title: 'Get Mandate Details', description: 'Retrieve mandate information', category: 'E-NACH' },
]
...
const quickActions: QuickAction[] = [
  { title: 'Test Payment Flow', description: 'Create a sample payment and test the complete flow', action: 'Try Now', category: 'Payment Gateway' },
  { title: 'Webhook Testing', description: 'Test webhook endpoints with sample payloads', action: 'Test Webhooks', category: 'Webhooks' },
  { title: 'E-NACH Simulation', description: 'Simulate E-NACH mandate creation and debit', action: 'Simulate', category: 'E-NACH' },
]

export default function PlaygroundPage() {
  const [activeSection, setActiveSection] = useState<PlaygroundSection>('overview')
  const [selectedCategory, setSelectedCategory] = useState('payments')

  const filteredEndpoints = apiEndpoints.filter((endpoint) => endpoint.category.toLowerCase() === selectedCategory)
          ...
          <>
            <div className="mb-8 grid gap-4 md:grid-cols-3">
              {runtimeSignals.map((signal) => {
                ...
                return (
                  <Card key={signal.title} className="border border-border/60 bg-background/80 shadow-lg">
                    <CardContent className="flex items-start gap-4 p-5">
                      <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary">
                        <Icon className="h-5 w-5" />
                      </span>
                      <span className="space-y-1">
                        <span className="text-xs uppercase tracking-wide text-muted-foreground">{signal.title}</span>
                        <span className="block text-lg font-semibold text-foreground">{signal.value}</span>
