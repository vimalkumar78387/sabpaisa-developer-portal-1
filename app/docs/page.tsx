...
const flutterRequestFormat: IntegrationKitDoc['requestFormat'] = {
  ...
  bodyNotes: [
    ...
    'Set Android minSdkVersion ≥ 19 in android/app/build.gradle and iOS deployment target ≥ 10.0 in Podfile.',
...
const webIntegrationKits: IntegrationKitDoc[] = [
  buildKit('reactjs', 'ReactJS', {
    ...
    useCases: [
      'React landing pages selling products or collecting fees with a “Pay with SabPaisa” CTA',
    ...
    setup: [
      'Step 1 – Install sabpaisa-pg-dev via npm install sabpaisa-pg-dev',
      'Step 2 – Create SabPaisaPaymentForm.js that wraps the PaymentForm component and forwards props plus env',
      'Step 3 – In App.js pass formData (client code, trans username/password, authKey, authIV, callbackUrl, clientTxnId, payer info, amount, channelId, init URL)',
      'Step 4 – Start the React app with npm start and toggle env between stag/prod as needed',
      'Step 5 – Inspect callbackFunction to process the SabPaisa response (status, codes, sabpaisaTxnId) and update UI/backend'
    ],
  ...
  buildKit('angularjs', 'Angular', {
    ...
    setup: [
      'Step 1 – Install sabpaisa-pg-sdk (npm install sabpaisa-pg-sdk)',
      'Step 2 – Configure environment.ts with clientCode, transUserName, transUserPassword, authKey, authIV, callbackUrl',
      'Step 3 – Define routes for PaymentFormComponent (/) and ResponsePageComponent (/response)',
      'Step 4 – Build PaymentFormComponent that collects payer data and calls submitPaymentForm(formState)',
      'Step 5 – Create ResponsePageComponent that runs parsePaymentResponse(authKey, authIV) and lists key/value pairs',
      'Step 6 – Run npm start, use env="STAG" for staging and switch to env="PROD" for live traffic'
    ],
...
const hybridIntegrationKits: IntegrationKitDoc[] = [
  buildKit('flutter', 'Flutter', {
    ...
    prerequisites: [
      ...
      'iOS: Podfile deployment target ≥ 10.0, ENABLE_BITCODE = YES, Swift version 5.0, CocoaPods + use_frameworks! configured.',
      'sabpaisa_flutter:^0.2.2 dependency declared in pubspec.yaml with flutter packages get executed.'
    ],
    setup: [
      'Installation – open the Flutter project, add sabpaisa_flutter:^0.2.2 in pubspec.yaml, then run flutter packages get.',
      'Android configuration – update android/app/build.gradle minSdkVersion 19+, add Proguard keep rules if obfuscating, run flutter clean && flutter pub get to sync Gradle.',
      'iOS configuration – set platform :ios, \'10.0\' in Podfile, enable Bitcode/Swift 5 via post_install hook, ensure use_frameworks!, then run pod install (rerun if headers are missing).',
      'Usage step 1 – import \'package:sabpaisa_flutter/sabpaisa_flutter.dart\' and instantiate SabPaisa sabPaisa = SabPaisa().',
      'Usage step 2 – register sabPaisa.on(SabPaisa.EVENT_PAYMENT_SUCCESS/ERROR, handler) to capture TransactionResponsesModel payloads.',
      'Usage step 3 – build the payment options map (first_name, last_name, currency, mobile_number, email_id, client_code, user_name, password, aes_key, aes_iv, env, txn_id, amount, callback_url, etc.).',
      'Usage step 4 – invoke sabPaisa.open(getPaymentOptions()) to launch checkout; handle callbacks to update order state.',
      'Troubleshooting – raise minSdkVersion if Gradle complains, bump iOS deployment target if Pods require it, ensure use_frameworks! when sabpaisa_flutter Swift headers are missing.'
    ],
  ...
  buildKit('react-native', 'React Native', {
    ...
    setup: [
      ...
      'Android: rely on auto-linking for RN ≥ 0.60 or run react-native link sabpaisa-react-lib-lite for older versions.',
